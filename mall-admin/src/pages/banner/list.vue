  ...
  <view class="banner-list-page">
    ...
    <view class="header-section">
      <view class="search-bar">
        ...
        <input
          ...
          @input="onSearchInput"
        />
        <view class="add-btn" @click="handleAdd">添加轮播图</view>
      </view>

      <view class="batch-bar" v-if="bannerList.length > 0">
    ...
    >
      ...
      <view class="loading-bar" v-if="loading && !isRefreshing">
        ...
        <text class="loading-text">正在加载轮播图数据...</text>
      </view>

      <!-- 轮播图列表 -->
      <view class="banner-list" v-if="bannerList.length > 0">
        <view
          class="banner-item"
        ...
        >
          ...
          <!-- 轮播图片展示 -->
          <view class="img-wrap" @click.stop="item.image ? previewImage(item.image) : null">
            <image
              class="banner-img"
              v-if="item.image"
              ...
              mode="aspectFill"
            ></image>
            <view class="banner-char" v-else>
              {{ item.title ? item.title.substring(0, 1) : '图' }}
            </view>
          </view>
          ...
          <view class="banner-info">
            <view class="title-row">
              ...
              <text class="banner-status-badge" :class="{ 'status-off': item.status === 0 }">
                {{ item.status === 1 ? '启用' : '禁用' }}
              </text>
            </view>
          </view>
...
export default {
  ...
  computed: {
    isAllSelected() {
      return this.bannerList.length > 0 && this.bannerList.every(item => this.selectedIds.includes(item.id))
}
    },
  async onLoad() {
    this.loadBanners()
  ...
  methods: {
    handleAdd() {
      uni.navigateTo({
        url: '/pages/banner/edit'
      })
    },
    handleEdit(item) {
      uni.navigateTo({
        url: `/pages/banner/edit?id=${item.id}`
      })
    },
    ...
    async loadBanners() {
      this.loading = true
      try {
        const res = await api.getBannerPage({
    ...
    toggleSelectAll() {
      if (this.isAllSelected) {
        ...
        this.selectedIds = this.bannerList.map(item => item.id)
}
    },
    clearSelection() {
      this.selectedIds = []
    ...
    onBatchActionChange(e) {
      ...
      if (this.selectedIds.length === 0) {
        uni.showToast({ title: '请先勾选需要操作的轮播图', icon: 'none' })
        return
      }
      if (action === 'enable') {
    ...
    handleToggleStatus(item, targetStatus) {
      ...
      uni.showModal({
        title: '提示',
        content: `确定${statusText}轮播图【${item.title || '未命名'}】吗?`,
        success: async (res) => {
          if (res.confirm) {
            try {
              await api.updateBannerStatus(item.id, targetStatus)
    ...
    handleBatchStatus(targetStatus) {
      ...
      uni.showModal({
        title: '提示',
        content: `确定批量${statusText}选中的 ${count} 个轮播图吗?`,
        success: async (res) => {
          if (res.confirm) {
            try {
              await api.batchUpdateBannerStatus(this.selectedIds, targetStatus)
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              ...
              if (this.bannerList.length === 1 && this.pageNum > 1) {
                this.pageNum--
              }
              this.loadBanners()
            } catch (e) {
    ...
    handleBatchDelete() {
      ...
      uni.showModal({
        title: '提示',
        content: `确定删除选中的 ${deleteCount} 个轮播图吗?`,
        success: async (res) => {
          if (res.confirm) {
            try {
              await api.batchDeleteBanner(this.selectedIds)
              uni.showToast({ title: '批量删除成功', icon: 'success' })
              this.selectedIds = []
              if (this.bannerList.length <= deleteCount && this.pageNum > 1) {
                this.pageNum--
              }
              this.loadBanners()
            } catch (e) {
...
.banner-status-badge.status-off {
  background: #f5f5f5;
  color: #999;
  border-color: #d9d9d9;
}
